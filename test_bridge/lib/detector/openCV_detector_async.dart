...
import 'openCV_detector.dart' as openCVDetector;
...
class ArucoDetectorAsync {
  ...
  void _initDetectionThread() async {
    ...
    final initReq = openCVDetector.InitRequest(toMainThread: fromDetectorThread.sendPort, markerPng: bytes);
    _detectorThread = await Isolate.spawn(openCVDetector.init, initReq);
  }

  Future<Float32List?> detect(CameraImage image, int rotation) {
    if (!arThreadReady) {
      print("sending retuin");
    ...
    _cbs[reqId] = res;
    var msg = openCVDetector.Request(
      reqId: reqId,
      method: 'detect',
  ...
  void destroy() async {
    ...
    var res = Completer();
    _cbs[reqId] = res;
    var msg = openCVDetector.Request(reqId: reqId, method: 'destroy');
    _toDetectorThread.send(msg);
  ...
  void _handleMessage(data) {
    ...
    if (data is openCVDetector.Response) {
